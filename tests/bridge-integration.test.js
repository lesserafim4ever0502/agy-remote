...
async function poll(url, options, attempts = 60) {
  ...
  for (let i = 0; i < attempts; i += 1) {
    try { const r = await fetch(url, options); if (r.ok || r.status === 401) return r; last = new Error(`HTTP ${r.status}`); }
    catch (e) { last = e; }
    await wait(50);
  }
...
test('bridge boots against a mock Language Server and exposes authenticated API', async (t) => {
  const mock = http.createServer((req, res) => {
    const method = req.url.split('/').pop();
    if (method === 'StreamAgentStateUpdates') {
      res.setHeader('content-type', 'application/connect+json');
      const update = buildConnectEnvelope({ update: {
        conversationId: 'c1',
        trajectoryId: 't1',
        status: 'CASCADE_RUN_STATUS_RUNNING',
        mainTrajectoryUpdate: { stepsUpdate: {
          indices: [0], totalLength: 1,
          steps: [{ status: 'CORTEX_STEP_STATUS_GENERATING', plannerResponse: { modifiedResponse: 'stream-ok', thinking: 'private' } }]
        } }
      } });
      res.write(update);
    ...
    const responses = {
      ...
      GetAllCascadeTrajectories: { trajectorySummaries: [{ key: 'c1', value: { summary: 'Demo', stepCount: 2, status: 'CASCADE_RUN_STATUS_IDLE', workspaces: [{ workspaceFolderAbsoluteUri: 'file:///repo' }] } }] },
      GetCascadeModelConfigData: { clientModelConfigs: [{ label: 'Demo Model', modelOrAlias: { model: 'MODEL_DEMO' }, quotaInfo: { remainingFraction: 0.5 } }] },
  ...
  const streamedEvent = await new Promise((resolve, reject) => {
    const ws = new WebSocket(`ws://127.0.0.1:${bridgePort}/api/v1/events?token=integration-token`);
    const timer = setTimeout(() => { ws.close(); reject(new Error('websocket stream timeout')); }, 3000);
    ws.addEventListener('open', () => ws.send(JSON.stringify({ type: 'subscribe', channel: 'conversation', resourceId: 'c1' })));
